...
4. **传输无关** —— 不绑定特定传输层，支持 stdio、HTTP SSE、WebSocket 等
...
Claude Code 支持三种 MCP 传输类型来连接外部 MCP Server：
...
#### SSE 类型：HTTP Server-Sent Events 连接

适用于远程 MCP Server 或需要长连接的场景。

```bash
# 添加一个 SSE 类型的 MCP Server
claude mcp add --transport sse my-remote-mcp https://mcp.example.com/sse
```
...
Claude Code                    MCP Server (HTTP)
    │                               │
    │── HTTP GET /sse ─────────────→│  (建立 SSE 连接)
    │←── SSE: endpoint info ────────│  (返回 POST 端点)
    │                               │
...
**优势**：支持远程部署、跨网络访问、服务端推送
**适用**：远程服务、共享型 MCP Server

#### SDK 类型：程序化集成

用于在代码中直接嵌入 MCP Client，适合复杂的程序化场景。
