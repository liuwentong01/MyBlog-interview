...
Claude Code 的工具扩展主要通过 **MCP Server 配置** 实现，而非传统意义上的"插件市场"。扩展工具的方式是：

- **手动配置**：在 `settings.json` 或通过 `claude mcp add` 命令注册 MCP Server
- **配置导入**：支持从 Claude Desktop 导入已有的 MCP 配置
- **动态加载**：通过 `--mcp-config` 参数在启动时加载额外的 MCP 服务器
- **作用域管理**：MCP Server 配置可以在 user / project 级别管理

```
┌─────────────────────────────────────┐
│        MCP Server 配置管理           │
│                                     │
│  claude mcp add <name> <command>    │  ← CLI 命令注册
│  settings.json → mcpServers: {...}  │  ← 配置文件
│  --mcp-config <path>                │  ← 启动时加载
│                                     │
│  作用域:                             │
...
> **重要说明**：Claude Code 目前没有中心化的"插件市场"（Marketplace）。MCP Server 需要用户手动配置或通过社区分享。以下描述的是 MCP Server 作为"插件"的工作方式。
...
- 通信方式支持：**stdio**（本地进程）、**Streamable HTTP**（远程服务，替代了旧的 HTTP+SSE 传输）
...
Agent (MCP Client)
    ...
    │
    └── Streamable HTTP ► Plugin B (MCP Server, 远程 HTTP)
```
...
#### 1.3 MCP Server 的生命周期

MCP Server 的管理是通过配置驱动的，而非传统的安装/卸载流程：

```
  [未配置]
     │
     │ claude mcp add / 编辑 settings.json
     ▼
  [已配置] ──── Claude Code 启动时自动连接
     │
     │ Claude Code 启动
     ▼
  [已连接/运行中] ──── initialize 握手 → tools/list 获取工具
     │
     │ Claude Code 退出 / MCP 进程崩溃
     ▼
  [已停止]
     │
     │ claude mcp remove / 编辑 settings.json
     ▼
  [未配置]
```
...
| 操作 | 方式 |
|------|------|
| **添加** | `claude mcp add <name> -- <command>` 或编辑 `settings.json` |
| **列出** | `claude mcp list` |
| **移除** | `claude mcp remove <name>` 或从 `settings.json` 删除 |
| **重连** | 重启 Claude Code 会自动重新连接所有配置的 MCP Server |
...
MCP Server 的权限主要通过以下方式控制：
- **环境变量隔离**：`env` 字段仅传递必要的环境变量
- **工具级权限**：通过 `allowedTools` / `blockedTools` 控制 MCP 工具的使用
- **用户确认**：MCP 工具默认需要用户确认才能执行
...
```json
{
  ...
  "allowedTools": [
    "mcp__risky-plugin__safe_tool"  // 仅允许特定的 MCP 工具自动执行
  ]
}
```
...
1. 来源验证
   └── MCP Server 来源是否可信？
       └── 项目配置（团队审核过的 .claude/settings.json）→ 基础信任
       └── 本地/自定义配置 → 需要用户显式授权
...
Agent Hooks:
  "pre-tool": [{ "matcher": "Bash", "command": "echo '即将执行 Bash 命令'" }]
...
```json
{
  "hooks": {
    "pre-tool": [
      { "matcher": "Bash", "command": "echo \"[$(date)] pre-tool: Bash with input: $TOOL_INPUT\" >> /tmp/agent-hooks.log" }
    ],
    "post-tool": [
      { "matcher": "Write", "command": "cd /Users/liuwentong/Code/R && npx prettier --write \"$TOOL_INPUT\" 2>/dev/null || true" },
      { "matcher": "Edit", "command": "cd /Users/liuwentong/Code/R && npx prettier --write \"$TOOL_INPUT\" 2>/dev/null || true" }
    ],
    "user-prompt-submit": [
      { "matcher": "*", "command": "echo \"[$(date)] User submitted prompt\" >> /tmp/agent-activity.log" }
    ],
    "init": [
      { "matcher": "*", "command": "echo '环境检查完成' && node --version && npm --version" }
    ]
  }
}
```
...
```json
{
  "hooks": {
    "post-tool": [
      { "matcher": "Write", "command": "prettier --write \"$TOOL_INPUT\"" },
      { "matcher": "Edit", "command": "prettier --write \"$TOOL_INPUT\"" }
    ]
  }
}
```
...
```json
{
  "hooks": {
    "post-tool": [
      { "matcher": "Write", "command": "npm test -- --findRelatedTests \"$TOOL_INPUT\" 2>/dev/null || true" },
      { "matcher": "Edit", "command": "npm test -- --findRelatedTests \"$TOOL_INPUT\" 2>/dev/null || true" }
    ]
  }
}
```
...
```json
{
  "hooks": {
    "post-tool": [
      { "matcher": "Bash", "command": "if echo \"$TOOL_INPUT\" | grep -q 'git push'; then osascript -e 'display notification \"代码已推送\" with title \"Agent\"'; fi" }
    ]
  }
}
```
...
```json
{
  "hooks": {
    "pre-tool": [
      { "matcher": "Bash", "command": "if echo \"$TOOL_INPUT\" | grep -qE 'rm -rf /|sudo'; then echo 'BLOCKED: 危险命令' >&2; exit 1; fi" }
    ]
  }
}
```
