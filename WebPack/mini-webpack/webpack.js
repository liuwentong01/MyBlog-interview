 ...
 * })();
 *
 * TODO 为什么用 require(entryId) 而不是直接内嵌入口代码？
...
${mod._source}
  ...
  /* ---------- 模块缓存 ----------
   * 用于保证每个模块只被执行一次（幂等性）,防止每次的执行结果不一样。
...
class Compilation {
  ...
  buildModule(name, modulePath) {
    ...
    //
    // NOTE： AST（抽象语法树）：把代码解析成树形结构，方便精准修改特定语法节点。
    // 如果用字符串替换，可能会误改注释、字符串等非 require 调用的地方。

    // 执行顺序：读文件 → Loader 转换 → 再解析 AST
    // 这个顺序是有意义的：
    // Loader 的输入可以是任何格式（TypeScript、JSX、Vue SFC、SCSS...）
    // Loader 的输出必须是合法的 JS
    // AST 解析器（@babel/parser）只认 JS
  ...
  build(callback) {
    ...
    callback(null, { chunks: this.chunks, modules: this.modules, assets: this.assets }, this.fileDependencies);
  }
}
